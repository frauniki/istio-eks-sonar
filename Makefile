...
$(BINARIES): $(GO_FILES)
	CGO_ENABLED=0 go build -ldflags="-X github.com/frauniki/istio-eks-sonar.Revision=$(shell git rev-parse --short HEAD)" -o $(BINDIR)/sonar-$(GOOS)-$(GOARCH)$(SUFFIX) $(@:$(BINDIR)/%=$(ROOT_PACKAGE)/cmd/%)
